...
db.once("open", (err) => {
    if(err){
        res.status(500).json({ Error: 'Unable to connect to \'macros\' MongoDB Atlas Cluster using Mongoose.' });
    } else  {
        console.log('Successfully connected to \'macros\' MongoDB Atlas Cluster using Mongoose.');
    }
});
...
// CREATE model *****************************************
const createMacro = async (meal, dateMeal, calories, fat, protein, fiber, sugar) => {
